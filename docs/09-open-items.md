...
| 37 | **Distributor prices for the remaining service lines** — razor wire, gate automation, video intercom, access control, fire detection, radio, power backup, control room, entrance control (boom barriers, turnstiles, road blockers), screening (archways, X-ray) | `OPEN` | Equipment tables and packaged solutions on nine service pages | All nine now have a live service page with real copy, honest limits and an FAQ, and each says plainly that equipment prices are **not published yet** rather than showing an estimate (docs/08 Sprint 6: "do not publish market-research estimates as our prices"). The items are seeded and unpublished, so one price in admin publishes a row. Sprint 6 also wants at least two packaged Solutions per line with full BOMs, and those cannot be built until the components are priced — a BOM assembled from estimates would put an invented total on a page whose whole claim is that its totals are real |
